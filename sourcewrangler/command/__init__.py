...
All commands are classes decorated with @register_command('name'), which will make the command run with `sw name`. They should define the static specify_args method, which takes an argparse that the command should fill with arguments. It should also define an instance method named run, which will receive a Namespace generated by parsing the arguments using the generated argparse.
"""
